<template>
    <div class="popup-body dlh-theme">
...
<script setup lang="ts">
    ...
    import dlhData from './static/dlh.json';
    import dlhAircrafts from './static/dlhAircrafts.json';
    ...
    const aircrafts = computed<string[]>(() => {
        //@ts-ignore
        return selectedCallsign.value ? dlhAircrafts[selectedCallsign.value] : dlhAircrafts['DLH'];
    });

    const allRoutes = computed(() => dlhData.routes.sort((a, b) => FilterUtils.sortAlphabetically(a.icao, b.icao)));
...
<style scoped>
    ...
    .dlh-theme table tr:hover td {
        background-color: #FFD966;
    }

    .dlh-theme .search-button {
        background-color: #05164D;
        color: white;
        border: none;
        border-radius: 4px;
    ...
    .dlh-theme .search-button:hover {
        background-color: #F9BA00;
        color: white;
    }

    .dlh-theme .search-button:active {
        background-color: #E0A800; /* Keep the hover color */
        color: white;
        transform: scale(0.98); /* Slightly shrink the button */
    }

    .dlh-theme .search-button:disabled {
